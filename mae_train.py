...
def main(args):
    ...
    # model = create_model("vit_large_patch16_224", pretrained=False, num_classes=len(class_names)) 
    ...
    print('Load TransformerEncoder learned in MAE')
    print(msg)
    model.to('cuda')
    ...
    for epoch in range(args.epoch):
        train_loss, train_count = trainer.finetune(device, train_loader, model, criterion, optimizer, lr_scheduler, scaler, use_amp, epoch)
        # train_loss, train_count = trainer.saves_finetune(device, train_loader, model, criterion, optimizer, lr_scheduler, scaler, use_amp, epoch)
